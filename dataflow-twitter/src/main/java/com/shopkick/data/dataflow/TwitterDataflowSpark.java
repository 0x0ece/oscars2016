 ...
 */
public class TwitterDataflowSpark {

  static final String OUTPUT = "/tmp/out";  // Default output filename
  static final String KAFKA_TOPIC = "tweets";  // Default kafka topic to read from
  // static final String KAFKA_BROKER = "localhost:9092,localhost:9093,localhost:9094,localhost:9095,localhost:9096,localhost:9097,localhost:9098";  // Default kafka broker to contact
  ...
  static class MapValuesFn extends DoFn<KV<String, String>, String>{
   ...
   */
  public static interface TwitterDataflowOptions extends PipelineOptions, FlinkPipelineOptions, SparkStreamingPipelineOptions {
  ...
  public static void main(String[] args) {
    ...
    PipelineOptionsFactory.register(SparkStreamingPipelineOptions.class);

    TwitterDataflowOptions options = PipelineOptionsFactory.fromArgs(args).withValidation()
      .as(TwitterDataflowOptions.class);

    // SparkStreamingPipelineOptions options = PipelineOptionsFactory.fromArgs(args).withValidation()
    //   .as(SparkStreamingPipelineOptions.class);

    // SparkStreamingPipelineOptions options = (SparkStreamingPipelineOptions)SparkStreamingPipelineOptionsFactory.create();

    options.setAppName("TwitterDataflow");
    options.setRunner(SparkPipelineRunner.class);
    // options.setTimeout(1000L);
    options.setStreaming(true);
    // options.setSparkMaster("local[10]");
    ...
    pipeline
     // .apply(TextIO.Read.named("ReadLines").from(options.getInput()))
        .apply(KafkaIO.Read.from(StringDecoder.class,
          StringDecoder.class, String.class, String.class, Collections.singleton(options.getKafkaTopic()),
          kafkaParams))
     // .apply(Window.<String>into(FixedWindows.of(Duration.standardMinutes(1))))
        ...
        .apply("MapValues", ParDo.of(new MapValuesFn()))
    ...
    // entityCounts
        ...
        // .apply(TextIO.Write.named("WriteEntityCounts").to(options.getOutput()));
        // .apply(ConsoleIO.Write.from())
        ;

     // .apply(new CountEntities())

    // pipeline.run();
    EvaluationResult res = SparkPipelineRunner.create(options).run(pipeline);
    res.close();
    // res.getRDD().foreach(println);
  }
}
